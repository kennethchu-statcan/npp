...
doSimulations <- function(
    ...
    for (i in seq(1,n.iterations)) {
        ...
        if( (population.flag == "01" && (is.na(Y_total_hat_CLW) || Y_total_hat_CLW < 0 || Y_total_hat_CLW > 100000)) ||
            (population.flag == "02" && (is.na(Y_total_hat_CLW) || Y_total_hat_CLW < 0 || Y_total_hat_CLW > 400000)) ||
            (population.flag == "03" && (is.na(Y_total_hat_CLW) || Y_total_hat_CLW < 0 || Y_total_hat_CLW > 1000000))   )   
        {
            write.csv(
                x         = LIST.samples[['non.probability.sample']],
                file      = paste0("probability-sample-population-", population.flag, "-index-", i, ".csv"),
                row.names = FALSE
                );
            write.csv(
                x         = LIST.samples[['probability.sample']],
                file      = paste0("non-probability-sample-population-", population.flag, "-index-", i, ".csv"),
                row.names = FALSE
                );
        } 
        #else if (population.flag == "02" && (is.na(Y_total_hat_CLW) || Y_total_hat_CLW < 0 || Y_total_hat_CLW > 400000)) {

        #    }
        #else if (population.flag == "03" && (is.na(Y_total_hat_CLW) || Y_total_hat_CLW < 0 || Y_total_hat_CLW > 1000000)) {
