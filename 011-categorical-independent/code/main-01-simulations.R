...
visualizePopulation(
    ...
    target_density_limits        = 10000 * c(  -0.9,0.9),
    target_density_breaks        = format(x = 10000 * seq(-0.8,0.8,0.4), digits = 2),
...
test.nppCART.AIC_aggregate(
    ...
    DF.population         = DF.population,
    bin.width             = 2e6,           # 5e5,           # 3000,
    limits                = c(  0,4e8),    # c(  0,1e8),    # c(  0,6e6),    # c(  0,2e6),
    breaks                = seq(0,4e8,1e8) # seq(0,1e8,2e7) # seq(0,6e6,1e6) # seq(0,2e6,5e5)
    );
