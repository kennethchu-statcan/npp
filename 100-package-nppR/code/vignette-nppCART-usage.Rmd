...
We remark that the colour gradient in the above plot indicates
...
which is an SRSWOR from the population, with unit selection probability 0.1.
...
We instantiate an __nppCART__ object, with the two input data sets as follows:
...
We call the __nppCART$grow( )__ method to grow the classification tree
...
Note that the data frame (here, assigned to _DF.npdata.estimated.propensity_)
returned by __nppCART$get_npdata_with_propensity( )__ is in fact the original
input data frame _DF.non.probability_
(passed in to __nppCART__ via the input parameter _np.data_)
with a few additional columns augmented at the right.


### Appendix A. Assessment of results on the synthetic data sets above.

Here, we make direct comparison between the __nppCART__ estimated self-selection
