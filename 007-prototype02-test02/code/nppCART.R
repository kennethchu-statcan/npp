...
### zzz ###
#'
#' @param predictors zzz
#' @param np.data zzz
#' @param p.data zzz
#' @param weight zzz
#' @param min.cell.size zzz
#' @param min.impurity zzz
#' @param max.levels zzz
#'
...
nppCART <- function(predictors = base::colnames(np.data), np.data, p.data, weight, min.cell.size = 10, min.impurity = 0.095, max.levels = 10) {
    return( R6_nppCART$new(predictors, np.data, p.data, weight, min.cell.size, min.impurity, max.levels) )
}
    ...
    private = list(
        ...
        get_best_split = function(np.currentRowIDs,p.currentRowIDs) {
            ...
            if (    base::length(uniqueVarValuePairs) < 1 | 
                    base::all(base::lapply(X = impurities, FUN = function(x) { return( base::is.nan(x) ) })) | 
