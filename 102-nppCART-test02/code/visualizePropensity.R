...
visualizePropensity <- function(
    ...
    my.ggplot <- my.ggplot + theme(
        ...
        axis.text.x      = element_text(size = textsize.axis,  face = "bold", angle = 45, vjust = 1, hjust = 1),
    ...
    if (!inputIsNumeric) {
        x1.levels <- levels(DF.population$x1)
        x2.levels <- levels(DF.population$x2)
        ...
        my.ggplot <- my.ggplot + geom_point(
            data    = DF.input,
            mapping = aes(x = x1.jitter, y = x2.jitter, colour = p_hat),
            alpha   = 0.2
            );
